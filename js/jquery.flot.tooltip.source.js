(function($) {
    ...
    FlotTooltip.prototype.init = function(plot) {
        ...
        function plothover(event, pos, item) {
            ...
            // Checks if a number is between the other two
            var inRange = function(x, x1, x2) {
                if (x1 > x2) {
                    var temp = x2;
                    x2 = x1;
                    x1 = temp;
            ...
            if (item) {
                ...
                $.each(plot.getData(), function(i, series) {
                    ...
                    while (minIndex <= maxIndex) {
                        j = Math.floor((minIndex + maxIndex) / 2);

                        if (series.data[j][0] < pos.x) {
                            minIndex = j + 1;
                        } else if (series.data[j - 1][0] > pos.x) {
                            maxIndex = j - 1;
                        } else {
                            xBeforeIndexes[0] = j - 1;
                            xAfterIndexes[0] = j;
                            // Check if the previous point pair defined a vertical line.  If so, check if the current y position is
                            // between the range of those points.  Then do it for the next set of points.
                            if (j >= 2 && series.data[j - 2][0] === series.data[j - 1][0]
                                    && inRange(pos.y, series.data[j - 2][1], series.data[j - 1][1])) {
                                xBeforeIndexes[1] = j - 2;
                                xAfterIndexes[1] = j - 1;
                            } else if (j + 1 < series.data.length && series.data[j + 1][0] === series.data[j][0]
                                    && inRange(pos.y, series.data[j][1], series.data[j + 1][1])) {
                                xBeforeIndexes[1] = j;
                                xAfterIndexes[1] = j + 1;
                            } else {
                                xBeforeIndexes = xBeforeIndexes.splice(0, 1);
                                xAfterIndexes = xAfterIndexes.splice(0, 1);
                            }
                            break;
                        }
                    }

                    if (xAfterIndexes[0] === -1) {
                        plot.hideTooltip();
                        return;
                    }

                    var pointPrev, pointNext, xBeforeIndex, xAfterIndex;
                    for (var j = 0; j < xBeforeIndexes.length; j++) {
                        xBeforeIndex = xBeforeIndexes[j];
                        xAfterIndex = xAfterIndexes[j];
                        pointPrev = {x: series.data[xBeforeIndex][0], y: series.data[xBeforeIndex][1]};
